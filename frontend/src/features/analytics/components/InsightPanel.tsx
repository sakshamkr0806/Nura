...
export function InsightPanel({ insights }: InsightPanelProps) {
  return (
    <div className="grid gap-4">
      <h3 className="text-lg font-semibold px-1">Pattern Analysis</h3>
      {insights.map((insight, idx) => (
        <Card key={idx} className="overflow-hidden border-l-4" style={{ 
          borderLeftColor: insight.type === 'success' ? 'hsl(var(--primary))' : 
                          insight.type === 'warning' ? 'hsl(var(--destructive))' : 
                          'hsl(var(--accent))' 
        }}>
          <CardHeader className="p-4 pb-2">
            <div className="flex items-center gap-2">
              ...
              {insight.type === 'info' && <Info className="h-4 w-4 text-accent-foreground" />}
